...
{private String appURL="http://pan.baidu.com/share/link?shareid=1496095071&uk=1427289872";
	...
	private ImageView shareImg,voiceImg,searchBtn;
	private final int MAXINDEX=dc.length;
	private TextView itemDc,itemDcN,itemDcText;
...
//提交当前数据 
	...
	public void setViewPager(){
		for(changeNum=0;changeNum<MAXINDEX;changeNum++){
			View myView;
			myView=inflater.inflate(R.layout.item0,null);
			shareImg=(ImageView)myView.findViewById(R.id.item0Share);
		voiceImg=(ImageView)myView.findViewById(R.id.item0Voice);
			shareImg.setId(changeNum+1000);
			voiceImg.setId(changeNum+2000);	
			itemDc=(TextView)myView.findViewById(R.id.item0Dc);
			itemDcN=(TextView)myView.findViewById(R.id.item0DcN);
			itemDcText=(TextView)myView.findViewById(R.id.item0Text);
			itemDc.setText(dc[changeNum]);
			itemDcN.setText(dcN[changeNum]);
		itemDcText.setText(dcText[changeNum]);			
			shareImg.setOnClickListener(new OnClickListener(){
					...
					{	
						...
						for(changeNum=0;changeNum<MAXINDEX;changeNum++){
							//判断页面中素
							while(p1.getId()==changeNum+1000){
								itemShare(dc[changeNum],dcN[changeNum]);
								break;
